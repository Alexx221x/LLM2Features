...
{df.info()} ’df.sample()’ (data sample): {df.sample()} ‘df.corr()‘ (correlation for num. features): {df.corr()}
’Useful information about the data:’ { human desc of DATA (OPTIONAL)}
...
The target variable in the data is ’{TARGET_NAME}’, the quality metric is {’ROC-AUC’ if class else ’RMSE’}.
...
Feature generation example: {PYTHON FORMAT BLOCK}
